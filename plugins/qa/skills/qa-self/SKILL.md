...
- `dx:qa-scope` has created a QA sub-ticket and tests need running
...
- Linear ticket with test cases (from `dx:qa-scope` or manual)
...
```
dx:qa-self
├── Prereq: dx:qa-scope (creates the QA ticket with test cases)
