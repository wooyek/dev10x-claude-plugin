...
description: Post a rich session bookmark comment on a PR — captures session ID, review thread status, current state, and next steps so the next session can pick up where this one left off.
---
