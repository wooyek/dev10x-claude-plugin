...
14a. **Orchestration list formatting** — mandatory tool invocations
...
14b. **Bundled call spec references** — complex `AskUserQuestion` gates
    need a `tool-calls/ask-<purpose>.md` file with full call spec.
    Do NOT require bundled files for trivial calls (`TaskUpdate`,
    simple `TaskCreate`).
15. **Config file schema** — when a skill reads/writes structured config
...
16. **Resolution-order completeness** — when SKILL.md documents a
    ...
    a logic bug (WARNING).
17. **Conditional steps** — when a task branch produces no artifact,
...
19. **Decision gates enforcement** — verify: (a) enforcement marker
    present, (b) evals.json includes assertions for plain-text
    substitution detection, (c) evals include `gate*-uses-tool` signals.
20. **Task dependency cross-validation** — for orchestration skills with
